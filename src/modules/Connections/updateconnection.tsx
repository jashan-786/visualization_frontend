...
export const UpdateConnection = () => {
  ...
  const onUpdateConnection = async () => {
    if (!currentUser || !selectedConnection) {
      alert("Please select a user and a connection to update.");
      return;
    }

    if (!connectionInfo.trim()) {
      alert("Please enter connection information.");
      return;
    }

    try {
      const response = await axios.post(
        `${connectionUrl}/api/v1/updateconnection`,
        {
      ...
      alert(response.data.message || "Connection updated successfully");
        ...
        <div className="py-8 md:py-12 flex items-center justify-center bg-gray-100 min-h-screen px-4">
          <div className="w-full max-w-[95%] md:max-w-[600px] bg-white flex flex-col rounded-lg shadow-lg p-4 md:p-8">
            ...
            <div className="space-y-4">
              ...
              {/* Input to update connection info */}
              {selectedConnection && (
                <input
                  type="text"
                  placeholder="Update connection info"
